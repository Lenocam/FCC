function telephoneCheck(str) {
  var letterCheck = /^1?[-\s]?\(?([2-9]\d{2})\)?[-\s]?([2-9]\d{2})[-\s]?(\d{4})$/;
  return letterCheck.test(str) ? true : false;
...
//[^A-Za-z]\b[^-]1|\d{3}[-(]|\d{3}\d{4}/
//[^A-Za-z"]\b[^-]1?|\d{3}(-\( )? |\d{3}(-\) )?\d{4}/
//[^A-Za-z]\b[^-] 1{0,1}[( ]?\d{3}[-( ]?|\d{3}[-) ]?\d{4}
// /^(1{1,2}\s)?\(?\d{3}\)?[\s.-]?\d{3}[\s.-]?\d{4}$/
